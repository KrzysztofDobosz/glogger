      ...
      LoggingService
{
   ...
   public Boolean verify(String userName, String data)
   {
      ...
      NeuralNetwork nn = new NeuralNetwork(alphaM, betaM, 1.0, 0.6);
      Matrix res = nn.getOutput(input1);
      res.print(1, 10);
      res = nn.getOutput(input2);
      res.print(1, 10);
      res = nn.getOutput(input3);
      res.print(1, 10);
      res = nn.getOutput(input4);
      res.print(1, 10);
      ...
      for (int i = 0; i < lines.length; i++)
         values[i] = new Double(lines[i]);
      System.out.println("asdasfd " + values.length);

      byte[] m = Crypto.encrypt(matrix, Crypto.getKey());

      DatabaseConnector db = new DatabaseConnector(
            "jdbc:mysql://localhost/gloggerdb", "gloggeruser", "gloggerpass");
      db.setParams("krzycho", 2, m);
      double[][] mm = Crypto.decrypt(db.getParams("krzycho", 2), Crypto
            .getKey());
       ...
       * 
       * Matrix flp = new Matrix(Crypto.decrypt(db.getParams(userName, 1),
       * Crypto .getKey())); Matrix slp = new
       * Matrix(Crypto.decrypt(db.getParams(userName, 2), Crypto .getKey()));
       * NeuralNetwork nn = new NeuralNetwork(flp, slp, nnBeta, nnEta);
       * 
       * String[] lines = data.split(","); double[] values = new
       * double[lines.length]; double[][] matrix = { values };
       * 
       * for (int i = 0; i < lines.length; i++) values[i] = new
       * Double(lines[i]);
       * 
       * Matrix result = nn.getOutput(new Matrix(matrix)); if (result.get(1, 1)
       * > 0.5) return true; else return false;
       */
   }

   public void train(String userName, ArrayList<String> dataArray)
   {
      double[][] one =
      {
      { 1. } };
      ...
      Matrix flp = Matrix.identity(100, 2);
      ...
      {
         inputs.add(Matrix.random(100, 2));
